...
class Validator implements ValidatorContract
{
    ...
    protected $implicitRules = ['Accepted', 'AcceptedIf', 'Filled', 'Present', 'Required', 'RequiredIf', 'RequiredUnless', 'RequiredWith', 'RequiredWithAll', 'RequiredWithout', 'RequiredWithoutAll'];
    protected $dependentRules = ['After', 'AfterOrEqual', 'Before', 'BeforeOrEqual', 'Confirmed', 'Different', 'ExcludeIf', 'ExcludeUnless', 'ExcludeWithout', 'Gt', 'Gte', 'Lt', 'Lte', 'AcceptedIf', 'RequiredIf', 'RequiredUnless', 'RequiredWith', 'RequiredWithAll', 'RequiredWithout', 'RequiredWithoutAll', 'Prohibited', 'ProhibitedIf', 'ProhibitedUnless', 'Same', 'Unique'];
    ...
    public function addFailure($attribute, $rule, $parameters = [])
    {
        ...
        $this->messages->add($attribute, $this->makeReplacements($this->getMessage($attribute, $rule), $attribute, $rule, $parameters));
...
class MySqlGrammar extends Grammar
{
    protected $modifiers = ['Unsigned', 'Charset', 'Collate', 'VirtualAs', 'StoredAs', 'Nullable', 'Srid', 'Default', 'Increment', 'Comment', 'After', 'First'];
...
class Encrypter implements EncrypterContract, StringEncrypter
{
    ...
    public static function supported($key, $cipher)
    {
        ...
        return $cipher === 'AES-128-CBC' && $length === 16 || $cipher === 'AES-256-CBC' && $length === 32 || $cipher === 'AES-128-GCM' && $length === 16 || $cipher === 'AES-256-GCM' && $length === 32;
    }
    ...
    public function encrypt($value, $serialize = true)
    {
        ...
        $value = in_array($this->cipher, ['AES-128-GCM', 'AES-256-GCM']) ? \openssl_encrypt($serialize ? serialize($value) : $value, $this->cipher, $this->key, 0, $iv, $tag) : \openssl_encrypt($serialize ? serialize($value) : $value, $this->cipher, $this->key, 0, $iv);
        if ($value === false) {
    ...
    {
        return hash_equals($this->hash($payload['iv'], $payload['value'], $payload['tag'] ?? ''), $payload['mac']);
    }
